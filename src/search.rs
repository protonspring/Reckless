...
fn search<NODE: NodeType>(
    ...
    // Qsearch Dive
    if depth <= 0 {
        return qsearch::<NODE>(td, alpha, beta, ply, depth - 1);
    }
    ...
    if !NODE::PV
        ...
        && !tt_move.is_quiet()
    {
        return qsearch::<NonPV>(td, alpha, beta, ply, depth - 1);
    }
        ...
        && !tt_move.is_quiet()
    {
        ...
        while let Some(mv) = move_picker.next::<NODE>(td, true, ply) {
            ...
            let mut score = -qsearch::<NonPV>(td, -probcut_beta, -probcut_beta + 1, ply + 1, depth - 1);
...
fn qsearch<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, beta: i32, ply: isize, depth: i32 ) -> i32 {
    ...
    while let Some(mv) = move_picker.next::<NODE>(td, skip_quiets(best_score), ply) {
        ...
        if !is_loss(best_score) {
            ...
            if depth < -15 && mv.to() != td.board.recapture_square() {
                break;
            }
