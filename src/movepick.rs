...
impl MovePicker {
    ...
    fn score_noisy(&mut self, td: &ThreadData) {
        ...
        let opponent = !td.board.side_to_move();
        ...
        if td.board.in_check() {
            ...
            for entry in self.list.iter_mut() {
                ...
                entry.score = 16 * captured.value()
                    ...
                    + 3000 * td.board.pinners(opponent).contains(mv.to()) as i32;
            }
        }
    }
    ...
    fn score_quiet(&mut self, td: &ThreadData, ply: isize) {
        ...
        for entry in self.list.iter_mut() {
            ...
            if pt == PieceType::Bishop {
                entry.score += 8000 * (!Bitboard::LONG_DIAGS.contains(mv.from()) && Bitboard::LONG_DIAGS.contains(mv.to())) as i32;
            }
        }
    }
}
